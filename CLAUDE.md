...
### Phase 1b（完了）：ベース安定化 — `experiment_base.py`

| Factor | 水準 | 結果 |
|---|---|---|
| A: Z高さ | 0.6 / 0.8 mm | **0.8mm採用** |
| B: 印刷速度 | 10 / 15 mm/s | **15mm/s採用** |
| C: アプローチ | なし / あり（8mm） | **あり採用** |
| D: オーバーラップ | なし / あり（+8°） | なしがやや良い |

### Phase 1c（完了）：継ぎ目品質 — `experiment_seam.py`

| Factor | 水準 | 結果 |
|---|---|---|
| Unretract Over MM | 2 / 4 / 6 / 8 mm | **差が見えず → 変数から除外、8mm固定** |
| Overlap | 0° / 2° / 4° / 6° | **0°が最良** |

### Phase 1d（完了）：マイナスオーバーラップ — `experiment_seam2.py`

| Factor | 水準 | 結果 |
|---|---|---|
| Overlap | -6° / -4° / -2° / 0° | **-4°が最良** |
...
| パラメータ | 採用値 | 備考 |
|---|---|---|
| Z（RING_HEIGHT） | **0.8 mm** | |
| 印刷速度 | **15 mm/s** | |
| アプローチ距離 | **8 mm** | ラジアルアプローチ |
| アンリトラクト分散距離 | **8 mm**（アプローチ全体） | その場アンリトラクトをしない |
| オーバーラップ | **-4°**（360°の4°手前で終了） | |
| 残課題 | ダマが依然わずかにある | リトラクト量の調整で将来対処 |

---

### Phase 2（進行中）：1ストランドの空中造形 — `experiment_phase2.py`

#### 固定パラメータ（現在値）

| パラメータ | 値 | 備考 |
|---|---|---|
| CIRCLE_DIA | 30mm | R=15mm |
| N_OSC_PER_REV | 8 | スパン11.8mm、傾斜角21.5° |
| LAYER_HEIGHT | 3.65mm | Z_AMP=±2.325mm |
| N_RINGS | 2 | R=13.2mm + R=15.0mm（剥がれ対策） |
| RING_DWELL_MS | 5000ms | リング固化待ち |
| FLOW_MESH | 100% | ※現在調査中 |
| Fan | 100% | メッシュ印刷中 |

#### レイアウト上の制約

- **X軸上に長いファンがある** → 実験はY軸方向に展開（X=150固定）
- X方向に展開するとファンと衝突する

#### N_OSC_PER_REVの制約

- N_OSC=12では印刷済みストランドにノズルが物理的に衝突した
- N_OSC=8（21.5°）が現在の安全値
- **N_OSC=9〜11（23.9°〜28.5°）は未検証**。成立する可能性あり → 要実験

#### テスト結果

| Speed | Dwell | FLOW_MESH | 結果 |
|-------|-------|-----------|------|
| 1.5mm/s | 1500ms | 50% | 途中で垂れる |
| 4.5mm/s | 1500ms | 50% | 途中で垂れる |
| 0.5mm/s | 3000ms | 50% | Dwellを増やしても改善しない |
| 0.5mm/s | 6000ms | 50% | 同上 |
| 1.5mm/s | 3000ms | **100%** | 結果待ち |

#### 現在の未解決問題

- **ヒゲ（stringing）がひどい**：RETRACT_MM=3.0mmでは不足の可能性。5.0mmを試す価値あり
- **印刷がキレイじゃない**：原因未特定
- **垂れ（droop）**：スパン11.8mm・角度21.5°の幾何制約内でフロー・速度・温度を探っている

#### 次の候補実験

- N_OSC=9〜11で角度上限を確認
- RETRACT_MM=5.0mmでヒゲを抑制
- NOZZLE_TEMP=190°Cで冷却改善
...
- [ ] Phase 2: FLOW_MESH=100% の結果を確認
- [ ] Phase 2: N_OSC=9〜11 で衝突限界と造形品質を探る
- [ ] Phase 2: ヒゲ対策（RETRACT_MM、NOZZLE_TEMP）
