...
c('G28                    ; Home all')
c('M106 S0                ; Fan OFF (ensure off regardless of previous print)')
...
c('; --- Purge line ---')
c('G1 Z10 F3000           ; Lift')
c('G1 X20 Y50 F6000       ; Move to purge start')
c('G1 Z0.8 F2000          ; Lower to 0.8mm (44% of 1.8mm nozzle dia)')
c('G92 E0                 ; Reset extruder')
c('G1 E10 F100            ; Slow prime: clear cold plug at nozzle tip')
c('G1 X160 E55 F400       ; Purge line (6.7mm/s, 140mm long)')
c('G1 X180 F5000          ; Wipe')
c('G1 E-5 F600            ; Retract before lift')
c('G92 E0                 ; Reset extruder')
c('')
...
def emit_path(xs, ys, zs, e_rate, label, flow, f_print=None, apexes=None):
    ...
    c(f'M221 S{flow}')
    # Travel to start
    c('G1 E-4 F600            ; Retract')
    c(f'G1 Z{zs[0]+2:.3f} F{F_TRAVEL}   ; Lift')
